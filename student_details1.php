...
// Query the database to fetch the student's details along with the mentor's name
$sql = "SELECT s.*, ss.sponsor_id, ss.sponsor_name, sm.mentor_name as mentor_name
        FROM student AS s
        LEFT JOIN student_sponsor AS ss ON s.id = ss.student_id
        LEFT JOIN student_mentors AS sm ON s.id = sm.student_id
        WHERE s.id = ?";
...
if ($stmt) {
    ...
    if ($row = mysqli_fetch_assoc($result)) {
        // Display student details along with the mentor's name in a two-column table
        ...
        echo '<tr><th>Mentor Name</th><td>' . $row['mentor_name'] . '</td></tr>';
        echo '</table>';
