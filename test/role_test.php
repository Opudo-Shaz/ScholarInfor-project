...
class Role
{
    ...
    public static function insertUserRoles($user_id, $roles)
    {
        ...
        // Step 1: Get the existing roles for the user from the database
        $existingRoles = array();
        ...
        // Step 2: Compare the existing roles with the roles to be assigned
        $rolesToAdd = array();
        foreach ($roles as $role_id) {
            if (!in_array($role_id, $existingRoles)) {
                $rolesToAdd[] = $role_id;
            }
        }

        // Step 3: Insert the roles that the user doesn't already have
        if (!empty($rolesToAdd)) {
            $values = array();
