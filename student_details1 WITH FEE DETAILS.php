...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $sql = "SELECT s.*, ss.sponsor_id, ss.sponsor_name
            FROM student AS s
            LEFT JOIN student_sponsor AS ss ON s.id = ss.student_id
            WHERE s.id = ?";
    ...
    if ($stmt) {
        ...
        if ($row = mysqli_fetch_assoc($result)) {
            ...
            echo '<tr><th>Guardian Names</th><td>' . $row['guardian_first_name'] . ' ' . $row['guardian_last_name'] . '</td></tr>';
            echo '<tr><th>Expected Completion Date</th><td>' . $row['expected_completion_date'] . '</td></tr>';
            echo '<tr><th>Student Status</th><td>' . $row['student_status'] . '</td></tr>';
            echo '<tr><th>Dropout Reason</th><td>' . $row['dropout_reason'] . '</td></tr>';
            echo '<tr><th>Sponsor ID</th><td>' . $row['sponsor_id'] . '</td></tr>';
            echo '<tr><th>Sponsor Name</th><td>' . $row['sponsor_name'] . '</td></tr>';
            echo '</table>';

// ... (Previous code remains unchanged)

// Query to fetch fee details for the student
$feeSql = "SELECT id, academic_year, term, fee_amount, fee_status, total_paid FROM school_fees WHERE student_id = ?";
$feeStmt = mysqli_prepare($conn, $feeSql);

if ($feeStmt) {
    mysqli_stmt_bind_param($feeStmt, "i", $studentId);
    ...
    // Display fee details for the student
    echo '<h2>Fee Details</h2>';
    echo '<table class="table table-bordered">';
    echo '<tr><th>Academic Year</th><th>Term</th><th>Fee Amount</th><th>Total Paid</th><th>Fee Status</th><th>Fee Balance</th></tr>'; // Header row

    while ($feeRow = mysqli_fetch_assoc($feeResult)) {
        // Calculate fee balance
        $feeAmount = $feeRow['fee_amount'];
        $totalPaid = $feeRow['total_paid'];
        $feeBalance = $feeAmount - $totalPaid;

        // Automatically change fee status to "Paid" if total paid is greater than or equal to fee amount
        if ($totalPaid >= $feeAmount) {
            $feeStatus = 'Paid';

            // Update the fee status in the database
            $updateSql = "UPDATE school_fees SET fee_status = ? WHERE id = ?";
            $updateStmt = mysqli_prepare($conn, $updateSql);
            if ($updateStmt) {
                mysqli_stmt_bind_param($updateStmt, "si", $feeStatus, $feeRow['id']);
                mysqli_stmt_execute($updateStmt);
            }
        } else {
            $feeStatus = $feeRow['fee_status'];
        }
        ...
        // Display fee details in table rows
        echo '<tr>';
        echo '<td>' . $feeRow['academic_year'] . '</td>';
        echo '<td>' . $feeRow['term'] . '</td>';
        echo '<td>$' . number_format($feeAmount, 2) . '</td>';
        echo '<td>$' . number_format($totalPaid, 2) . '</td>';
        echo '<td>' . $feeStatus . '</td>';
        echo '<td>$' . number_format($feeBalance, 2) . '</td>';
        echo '</tr>';
    }

    // Determine if the total balance is "Overdue" or "Overpaid"
    $totalFeeStatus = $totalFeeBalance > 0 ? 'Overdue' : ($totalFeeBalance < 0 ? 'Overpaid' : '');

    echo '<tr>';
    echo '<th>Total</th>';
    echo '<th></th>';
    echo '<th>$' . number_format($totalFeeAmount, 2) . '</th>';
    echo '<th></th>';
    echo '<th>' . $totalFeeStatus . '</th>';
    echo '<th>$' . number_format($totalFeeBalance, 2) . '</th>';
    echo '</tr>';
}
