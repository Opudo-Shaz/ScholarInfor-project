                    ...
                    if (isset($_GET['id']) && is_numeric($_GET['id'])) {
                        ...
                        if ($stmt) {
                            ...
                            if ($row = mysqli_fetch_assoc($result)) {
                                ...
                                // Create a form to select a sponsor
                                echo '<form method="post" action="insert_student_sponsor.php">';
                                echo '<div class="form-group">';
                                echo '<label for="sponsorSelect">Select a Sponsor:</label>';
                                echo '<select class="form-control" id="sponsorSelect" name="sponsor_id">';
                                
                                // Query the database to fetch sponsor names and IDs
                                $sponsorSql = "SELECT id, sponsor_name FROM sponsors";
                                ...
                                if (mysqli_num_rows($sponsorResult) > 0) {
                                    while ($sponsorRow = mysqli_fetch_assoc($sponsorResult)) {
                                        echo '<option value="' . $sponsorRow['id'] . '">' . $sponsorRow['sponsor_name'] . '</option>';
                                    }
                                }
                                echo '</select>';
                                echo '</div>';
